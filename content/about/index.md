---
title: "About"
layout: "single"
ShowToc: false
ShowReadingTime: false
ShowBreadCrumbs: false
---

## whoami

I'm **vataseradu** (Vatase Radu) — a cybersecurity enthusiast, academic researcher, and CTF player.

This blog is where I share my work across several areas:

- **Cybersecurity Research** — vulnerability analysis, offensive security, penetration testing
- **CTF Writeups** — solutions and walkthroughs from capture-the-flag competitions
- **Academic Work** — university and master's level research papers and notes
- **Tools & Projects** — custom scripts, tools, and open-source contributions

---

## whoami (RO)

Sunt **vataseradu** (Vatase Radu) — pasionat de cybersecurity, cercetător academic și jucător CTF.

Pe acest blog public lucrările mele din mai multe arii:

- **Cercetare Cybersecurity** — analiză de vulnerabilități, securitate ofensivă, penetration testing
- **Writeup-uri CTF** — rezolvări și walkthrough-uri de la competiții capture-the-flag
- **Lucrări Academice** — articole de cercetare, notițe și proiecte de la facultate și master
- **Tools & Proiecte** — scripturi custom, unelte și contribuții open-source

## Contact

- GitHub: [vataseradu](https://github.com/vataseradu)
